...
class MSDeformableAttention(nn.Module):
    ...
    def _reset_parameters(self):
        ...
        if self.decoupled:
            init.constant_(self.attention_weights_pos.weight, 0)
            init.constant_(self.attention_weights_pos.bias, 0)
    ...
    def forward(self,
                ...
                prev_ca_output: torch.Tensor = None,
                amr_gamma: torch.Tensor = None):
        """
        Args:
            ...
            query_pos_embed (Tensor, optional): [bs, query_length, C] — DP-CA: positional query
            prev_ca_output (Tensor, optional): [bs, query_length, C] — AMR: previous layer's
                cross-attention aggregated output, used as a spatial memory hint.
            amr_gamma (nn.Parameter, optional): Learnable scalar for AMR residual scale.

        Returns:
            output (Tensor): [bs, Length_{query}, C]
            attention_weights (Tensor): [bs, query_length, num_heads, total_points] — for AMR passthrough
        """
        bs, Len_q = query.shape[:2]

        if self.decoupled and query_pos_embed is not None:
            sampling_offsets: torch.Tensor = self.sampling_offsets(query)
            attention_weights = self.attention_weights(query) + self.attention_weights_pos(query_pos_embed).type_as(query)
        else:
            sampling_offsets: torch.Tensor = self.sampling_offsets(query)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        # cross attention (dfine version: AMR not used here, ignore attn_weights)
        target2, _ = self.cross_attn(\
